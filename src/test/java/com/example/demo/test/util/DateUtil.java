...
//    }
    ...
    public static List<String> getDates(String begin, String end, String parsePatterns) {
        List<String> dates = new ArrayList<>();
        Date beginDate = parseDate(begin, parsePatterns);
        Date endDate = parseDate(end, parsePatterns);
        if (endDate.compareTo(beginDate) == 0) {
            dates.add(begin);
            return dates;
        }
        while (beginDate.compareTo(endDate) <= 0) {
            dates.add(FastDateFormat.getInstance(parsePatterns).format(beginDate));
            beginDate = getNextDay(beginDate);
        }
        return dates;
    }
